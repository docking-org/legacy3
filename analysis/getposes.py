...
def getPosesCache(file, molnum, id, rank, cache=True):
  ...
  if os.path.exists(bsFile):
    if os.path.getsize(bsFile) > 0:
      ...
      if (okay not in cacheDict) or (not marshal.loads(cacheDict[okay])):
...
def getPoses(indir, dockdir, output, idfile, topX, receptors, rankfile, part, \
  ...
  for anId in idlist:
    ...
    if len(scoreList) > 0:
      for position in range(min(topXfind, len(scoreList))): #only do the topX 
        ...
        for oneLine in thisMol2:
          if oneLine.find("##########        Ligand Energy:") != -1:
            ...
            if bbrw:
              ...
              outputFile.write("##########                BBdof: " + \
                  str(dof) + '\n')
              partKeysBlackBox = list(partPrefsBlackBox.keys())
          ...
          if oneLine.find("@<TRIPOS>MOLECULE") != -1:
            ...
            if zincData: #before the tripos molecule line, write vendor info
              zincSubstance = zincData.substances.get(anId, fields=list(ZINC_FIELD_MAP.keys()))
              for key, field in list(ZINC_FIELD_MAP.items()):
                value = zincSubstance[key]
                if isinstance(value, dict):
                  for vKey, vValue in list(value.items()):
