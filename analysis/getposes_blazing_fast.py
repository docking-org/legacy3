...
def main():
	...
	for chunk in chunk_list:
		...
		gz_file = docking_dir+chunk+"/test.mol2.gz"			
                ...
                #checkcmd = "gzip -t "+db2_gz_file
                #checkcmd = "gzip -t "+gz_file
                checkcmd = "gzip -t "+gz_file+" |& awk '{print $0}'"
                #outputcmd = os.popen(checkcmd).readlines()[0]
                fhcmd = os.popen(checkcmd)
                outputcmd = ''
                for linecmd in fhcmd: 
                     #print linecmd
                     outputcmd=outputcmd+linecmd.strip()
                
                #print "here is the command: "+ checkcmd
                #print "here is an error measage: "+ outputcmd

                if (outputcmd!=''):
                    #print db2_gz_file+" has a problem ..."
                    print(("here is the command: "+ checkcmd))
                    print(("here is an error measage: "+ outputcmd))
                    print((gz_file+" has a problem ..."))
