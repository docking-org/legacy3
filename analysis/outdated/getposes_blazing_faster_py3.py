...
def process_gz(filename, lig_dict, chunk):
	...
	for line in ligandFile.stdout:
		...
		if len(splt) > 2:
			if splt[0].decode('utf-8') == "##########" and splt[1].decode('utf-8') == "Name:" and better_energy == True:
		#		print(name, splt[2], comp_found, better_energy, temp_tote, new_tote)
			...
			if splt[0].decode('utf-8') == "##########" and splt[1].decode('utf-8') == "Name:" and better_energy == False:
				name = splt[2].decode('utf-8')
				print(name)
				if name in lig_dict:
					comp_found = True
			...
			if splt[0].decode('utf-8') == "##########" and splt[1].decode('utf-8') == "Total" and splt[2].decode('utf-8') == "Energy:" and comp_found == True:
				temp_tote = lig_dict[name][0]
				new_tote = float(splt[3].decode('utf-8'))
