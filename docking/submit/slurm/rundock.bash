#!/bin/bash
#SBATCH -J rundock
#SBATCH -o /dev/shm/rundock_%A_%a.out
#SBATCH -e /dev/shm/rundock_%A_%a.err
...
if [ -d $COMMON_DIR ]; then
	...
	if [ -f /dev/shm/${dockfileshash}.lock ] && ! [ -w /dev/shm/${dockfileshash}.lock ]; then
		cnd2="true"
...
LOG_OUT=/dev/shm/rundock_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out
LOG_ERR=/dev/shm/rundock_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
...
#(
#        flock -x 9
#        if ! [ -f $JOB_DIR/dockfiles/.fixed_indock ]; then
#                fix_indock $JOB_DIR/dockfiles/INDOCK
#        fi
#        flock -u 9
#)9>/dev/shm/dockfiles.${dockfileshash}.lock
...
wait $dockpid
sleep 5 # for some reason the bash script likes to jump the gun when sending DOCK a SIGUSR1, it goes ahead with cleanup before DOCK is finished!
# I'm not exactly sure what causes this, and frankly I don't care. sleep 5 is a low tech solution that works 
